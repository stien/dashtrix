...
class front_model extends CI_Model {
	...
	public function job_search_records($search,$cid){
		$query 	= $this->db->query("SELECT * FROM `dev_web_jobs` WHERE `jTitle` LIKE '%".$search."%' OR `jNature` = '".$cid."' ORDER BY `jID` DESC LIMIT 0,10");
		$row = $query->result_object();
		return $row;
	}
...
");
	...
	public function topviewedJobs(){
		$query = $this->db->query("SELECT * FROM `dev_web_jobs` WHERE `jJobStatus` = '1' AND `jview` != '0' ORDER BY `jview` DESC LIMIT 0,8");
		$row = $query->result_object();
		return $row;	
	}
