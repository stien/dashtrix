    ...
    <div class="col-xs-10 bxlogin nopad">
      <div class="login-inner2 left col-xs-6" style="min-height: 410px;">
       ...
       <form name="signupform" id="signupform" action="<?php echo base_url(); ?>jobsite/signup_recruiter_submit" method="post" onsubmit="return checksubmitdata()">
            ...
            <input type="text" name="companyurl" id="companyurl" required class="inputjob" onBlur="domainchecker()" autocomplete="off" placeholder="Company Website" value="<?php if(isset($_SESSION['wrongsignup'])){echo $_SESSION['wrongsignup']['companyurl'];} else {echo "www.";}?>" autocomplete="off">
          ...
          <div class="col-xs-12 nopad boxinput">
            <div class="col-xs-7 nopad">
            	<input type="text" name="emailaddpay" id="emailaddpay" onBlur="checkspaces('emailaddpay')" autocomplete="off" required class="inputjob" placeholder="Email Address">
            </div>
          ...
          <div class="col-xs-12 nopad boxinput">
            ...
            <div id="passinfobx" style="display:none;">
                	...
                	<div id="minlength" class="caseletter"><i class="fa fa-check-circle-o"></i> Be Atleast 8 characters</div>
          ...
          <div class="col-xs-12 nopad boxinput">
            <div class="col-xs-3 nopad">
				...
				?>
				  <option value="<?php echo $concode->phonecode;?>" <?php if($concode->iso == $countrycode){echo "SELECTED";}?>><?php echo $concode->iso;?> - <?php echo $concode->phonecode;?></option>
                <?php
					endforeach;
				?>
				</select>
          ...
          <div class="col-xs-12 nopad boxinput">
              ...
              <div class="col-xs-12 nopad boxinput wd100">
              <select name="countryrecruit[]" id="countryrecruit" multiple="multiple" required class="inputjob wd100">
          ...
          <div class="col-xs-12 nopad boxinput">
           <button type="submit" name="signup" id="signupbtnrec" class="btn pink right">Sign up</button>
          </div>
          </form>
...
function showboxpassinfo(){
	...
	if($('#passwordlogin').val().length>7 && $('#passwordlogin').val().match(number) && $('#passwordlogin').val().match(alphabetssmall) && $('#passwordlogin').val().match(alphabetsupper) && $('#passwordlogin').val().match(special_characters)){} else {
	$("#passinfobx").show();
	}
}
function passwordcheckerjobsite()
{
	...
	if($('#passwordlogin').val().length>7 && $('#passwordlogin').val().match(number) && $('#passwordlogin').val().match(alphabetssmall) && $('#passwordlogin').val().match(alphabetsupper) && $('#passwordlogin').val().match(special_characters)){
		$("#passinfobx").hide();
	}
	else {
			$("#passinfobx").show();
...
function checksubmitdata(){
	...
	var alphabetsupper = /([A-Z])/;
	var special_characters = /([~,!,@,#,$,%,^,&,*,-,_,+,=,?,>,<])/;
	$("[id='signupbtnrec']").attr("diabled",true);
	$("[id='signupbtnrec']").html("Please wait...");
	if($('#passwordlogin').val().length<8 || !$('#passwordlogin').val().match(number) || !$('#passwordlogin').val().match(alphabetssmall) || !$('#passwordlogin').val().match(alphabetsupper) || !$('#passwordlogin').val().match(special_characters)){
			 $("#passwordlogin").focus();
			 $("#passinfobx").show();
			 $("[id='signupbtnrec']").html("Signup");
			 $("[id='signupbtnrec']").attr("diabled",false);
			 return false;	 
		 } else {
	var regex = new RegExp("^(http[s]?:\\/\\/(www\\.)?|ftp:\\/\\/(www\\.)?|www\\.){1}([0-9A-Za-z-\\.:%_\+~#=]+)+((\\.[a-zA-Z]{2,3})+)(/(.)*)?(\\?(.)*)?"); 
var without_regex = new RegExp("^(www\\.)([0-9A-Za-z-\\.:%_\+~#=]+)+((\\.[a-zA-Z]{2,3})+)(/(.)*)?(\\?(.)*)?");
...
$('#countryrecruit').select2({  placeholder: "Select Country"
,language: {
    noResults: function (params) {
      return "Employers only from European countries allowed.";
    }
  }
}); 
});
