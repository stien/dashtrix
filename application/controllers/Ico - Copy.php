...
//          print_r($_SESSION['JobsSessions']);
            ...
            if($user[0]->uActType == 2 || $user[0]->uActType == 3){
                if($seg!="resend-otp" && $seg!="submit-otp" && $seg!="verify-otp" && $seg!="logout" && $seg!="login" && $seg!="accept-terms" && $seg!="resend-verification-email" && $this->uri->segment(3)!="account")
                    $this->check_if_verified();
            }
        }
        else {}
        $sid = session_id();
...
//    }
    ...
    public function do_register()
    {
        ...
        else
        {
            ...
            $rendomcode = $this->generateRandomString();
            $ref_id = $this->front_model->get_query_simple('*','dev_web_user',array('uCode'=>$this->input->post('ref')))->result_object();
            if(!empty($ref_id))
                $ref_id = $ref_id[0]->uID;
            else
                $ref_id = 0;
...
//                      'uAddress'      => $this->input->post('address'),
    ...
    public function send_verification($uID,$rendomcode)
    {
        ...
        $replacewith = array(WEBURL, $rendomcode, $user->uFname." ".$user->uLname,TITLEWEB,$user->uEmail,$this->input->post('password')?$this->input->post('password'):'******');
...
';
  ...
  <tr>
            ...
            if(ACTYPE == "1"){
                if(isset($_POST['type']))
                {
                    $array = array(
                        ...
                        'allowed_country'=>$this->input->post('allowed_country'),
                ...
                else
                {
                    ...
                    if($option->allowed_country!=0){
                        ...
                        if($query['countryCode'])
                        {
                            ...
                            if(!empty($country_id))
                            {
                                if($option->allowed_country!=$country_id[0]->id)
                                {
                                    $disabled = 3;
                                }
                                else
    ...
    public function take_campaign_image()
    {
        ...
        $max_width = "500";
        $return = array('status'=>0,'msg'=>"try again");
        $valid_formats = array("jpg", "png", "gif", "bmp","jpeg","doc","DOC","docx","DOCX","pdf","PDF",'rtf','RTF');
...
//           print_r($campaign);
    ...
    {
        $this->db->where('DATE(end_date) <',date("Y-m-d"))->where('end_on_end_date',1)->update('dev_web_ico_settings',array('active'=>0));
    ...
    public function banned_countries()
    {
        if(!isset($_SESSION['JobsSessions'])){
            ...
            if(ACTYPE == "1"){
                if(isset($_POST['submit']))
                {
                    $arr = array('cID'=>$this->input->post('country'));
...
//            $form     = $config->result_object();
    ...
    public function export_users($type="csv")
    {
        if(!isset($_SESSION['JobsSessions'])){
            header ( "Location:" . $this->config->base_url ()."login");
        }
    ...
    private function array2csv($list,$name="file",$type)
    {
        $this->load->helper('download');
    ...
    public function upload_wallet_addresses($id)
    {
        ...
        foreach($files_to['name'] as $key=>$name){
            ...
            if(strlen($name)) {
                ...
                if(in_array($ext,$valid_formats)) {
                    if(1==1) {
                        $actual_image_name = "c_".GUID().'_camp'.'_'.'.'.$ext;
                        $filePath = $path .'/'.$actual_image_name;
                        $tmp = $files['tmp_name'][$key];
                        if(move_uploaded_file($tmp, $filePath)) {
