...
//			print_r($_SESSION['JobsSessions']);
            ...
            if($user[0]->uActType == 2 || $user[0]->uActType == 3){
                if($seg!="resend-otp" && $seg!="submit-otp" && $seg!="verify-otp" && $seg!="logout" && $seg!="login" && $seg!="accept-terms" && $seg!="resend-verification-email")
...
//            $this->sendgrid_mail(array($this->input->post('email'),''),$message,$edata[0]->eName);
...

    {
...

        {
            ...
            $this->sendgrid_mail(array($_REQUEST['email'],''),$message,$edata[0]->eName);
        ...
        if(!isset($_SESSION['JobsSessions'])){
            ...
            if(ACTYPE == "1"){
                if(isset($_POST['submit']))
                {
                    ...
                    $this->front_model->add_query('dev_web_registration_form',$arr);
                ...
                {
                    $this->data['registration_form']=$this->front_model->get_query_simple('*','dev_web_registration_form',array())->result_object();
    ...
    public function remove_form_field($id)
    {
        if(!isset($_SESSION['JobsSessions'])){
            ...
            if (ACTYPE == "1") {
                ...
                $this->front_model->delete_query('dev_web_registration_form',array('id'=>$id));
...
//                    $this->data['publications']=$this->front_model->get_query_simple('*','dev_web_publications',array('status'=>1))->result_object();
    ...
    public function paid_content_placement_admin()
    {
        if(!isset($_SESSION['JobsSessions'])){
            ...
            if (ACTYPE == "1") {
                $this->data['paid_contents']=$this->front_model->get_query_simple('*','dev_web_paid_content',array())->result_object();
                $this->load->view('frontend/paid_contents',$this->data);
            }
        }
    }
    ...
    public function manage_content_promotions()
      {
            if(!isset($_SESSION['JobsSessions'])){
              header ( "Location:" . $this->config->base_url ()."login");
          }
