...
class front_model extends CI_Model {
	...
	public function get_jobs_list_home($limit, $start)

	{

		$query 	= $this->db->query("SELECT * FROM `dev_web_jobs` WHERE `jJobStatus` = '1' ORDER BY `jID` DESC LIMIT ".$limit.", ".$start."");
...
ON U.cID = P.catParent ORDER BY P.cID ASC");
...

	{
		...
		//echo "UPDATE `dev_web_reviews` SET `rCount` = '".$count."' WHERE `rID` = '".$rID."'";
	...
	public function updateuserprofilepic($count,$rID)

	{

		$query 	= $this->db->query("UPDATE `dev_web_user` SET `uImage` = '".$count."' WHERE `uID` = '".$rID."'");
...
");
	...
	public function checkifurlfound($url){

		//echo "SELECT * FROM `dev_web_user` WHERE `uWebsite` = '".$url."' AND `uActType` = '2'";

		$query = $this->db->query("SELECT * FROM `dev_web_user` WHERE `uWebsite` = '".$url."' AND `uActType` = '2'");
	...
	public function checkifurlfounddbadd($url){
	...
	public function checkselect2skills($url){

		//echo "SELECT * FROM `dev_web_user` WHERE `uWebsite` = '".$url."' AND `uActType` = '2'";

		$query = $this->db->query("SELECT `skVal` FROM `dev_web_skills` WHERE `skVal` LIKE '%".$url."%'");
	...
	public function agent_today_jobs($type)

	{

		$query 	= $this->db->query("SELECT * FROM `dev_web_jobs` WHERE  DATE(jPostedDate) = CURDATE() AND `agtID` = '".$type."'");
...
public function get_Saved_jobs_home()

	{

		$query 	= $this->db->query("SELECT * FROM `dev_web_saved` AS S, `dev_web_jobs` AS J WHERE S.`jID` = J.`jID` ORDER BY `sID` DESC LIMIT 0,9");
		return $query;

	}
	
	public function get_jobs_by_country()
	{

		$query 	= $this->db->query("SELECT * FROM `dev_web_jobs` GROUP BY `jCoutry` ORDER BY RAND() LIMIT 0,15");
		return $query;

	}
	
	public function totaltokensolds($period="today")
	{


		//today
		if($period=="today")
		$query 	= $this->db->query("SELECT SUM(`tokens`) AS TOKENS FROM `dev_web_transactions` where `status` = '2' AND DATE(datecreated) = '".date("Y-m-d")."'");

		elseif($period=="week")
		$query 	= $this->db->query("SELECT SUM(`tokens`) AS TOKENS FROM `dev_web_transactions` where `status` = '2' AND DATE(datecreated) >= '".date("Y-m-d",strtotime("-7 days"))."'");
	// echo $this->db->last_query();exit;

		elseif($period=="month")
		$query 	= $this->db->query("SELECT SUM(`tokens`) AS TOKENS FROM `dev_web_transactions` where `status` = '2' AND DATE(datecreated) >= '".date("Y-m-d",strtotime("-30 days"))."'");
		else
		$query 	= $this->db->query("SELECT SUM(`tokens`) AS TOKENS FROM `dev_web_transactions` where `status` = '2' ");
	...
	public function total_sales($period="today")
	{


		//today
		if($period=="today")
		$query 	= $this->db->query("SELECT SUM(`usdPayment`) AS TOKENS FROM `dev_web_transactions` where `status` = '2' AND DATE(datecreated) = '".date("Y-m-d")."'");

		elseif($period=="week")
		$query 	= $this->db->query("SELECT SUM(`usdPayment`) AS TOKENS FROM `dev_web_transactions` where `status` = '2' AND DATE(datecreated) >= '".date("Y-m-d",strtotime("-7 days"))."'");
	// echo $this->db->last_query();exit;

		elseif($period=="month")
		$query 	= $this->db->query("SELECT SUM(`usdPayment`) AS TOKENS FROM `dev_web_transactions` where `status` = '2' AND DATE(datecreated) >= '".date("Y-m-d",strtotime("-30 days"))."'");
		else
		$query 	= $this->db->query("SELECT SUM(`usdPayment`) AS TOKENS FROM `dev_web_transactions` where `status` = '2' ");
	...
	public function totaltransactions($period='today')
	{

		if($period=="today")
		$query 	= $this->db->query("SELECT SUM(`tokens`) AS TOKENS FROM `dev_web_transactions` where DATE(datecreated) = '".date("Y-m-d")."'");

		elseif($period=="week")
		$query 	= $this->db->query("SELECT SUM(`tokens`) AS TOKENS FROM `dev_web_transactions` where DATE(datecreated) >= '".date("Y-m-d",strtotime("-7 days"))."'");
	// echo $this->db->last_query();exit;

		elseif($period=="month")
		$query 	= $this->db->query("SELECT SUM(`tokens`) AS TOKENS FROM `dev_web_transactions` where DATE(datecreated) >= '".date("Y-m-d",strtotime("-30 days"))."'");
		else
		$query 	= $this->db->query("SELECT SUM(`tokens`) AS TOKENS FROM `dev_web_transactions` ");


		// $query 	= $this->db->query("SELECT count(*) AS TOKENS FROM `dev_web_transactions`");
		$row = $query->result_object();		
		return $row[0]->TOKENS;
	}
	public function totaltransactions_2($period='today')
	{

		if($period=="today")
			$trans = $this->db->where('DATE(datecreated)',date("Y-m-d"))->count_all_results('dev_web_transactions');
		

		elseif($period=="week")
			$trans = $this->db->where('DATE(datecreated) >=',date("Y-m-d",strtotime("-7 days")))->count_all_results('dev_web_transactions');
	

		elseif($period=="month")
			$trans = $this->db->where('DATE(datecreated) >=',date("Y-m-d",strtotime("-30 days")))->count_all_results('dev_web_transactions');
		else
			$trans = $this->db->count_all_results('dev_web_transactions');
		
		return $trans;
	}
	public function totalvisitors()
	{
		$query 	= $this->db->query("SELECT count(*) AS TOKENS FROM `dev_web_visitors`");
		$row = $query->result_object();		
		return $row[0]->TOKENS;
	}
	public function totalusers()
	{
		$query 	= $this->db->query("SELECT count(*) AS TOKENS FROM `dev_web_user`");
		$row = $query->result_object();		
		return $row[0]->TOKENS;
	}
